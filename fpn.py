...
class PyramidFeatures(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.c2_channels = cfg.c2_channels
        ...
        self.P2_1 = torch.nn.Conv2d(self.c2_channels, self.fpn_channels, kernel_size=1, stride=1, padding=0, bias=True)
        self.P2_2 = torch.nn.Conv2d(self.fpn_channels, self.fpn_channels, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, x):
        ...
        p3=self.P3_1(c3)
        p3=p3+p4_upsampled
        p3_upsampled=F.interpolate(p3,size=c2_shape,mode="nearest")
        p3=self.P3_2(p3)
