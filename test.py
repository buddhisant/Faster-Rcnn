...
def test(epochs_tested):
    ...
    model = Faster_RCNN(is_train=is_train)
    ...
    for epoch in epochs_tested:
        ...
        with torch.no_grad():
            for data in tqdm(dataloader):
                ...
                cls_pred, reg_pred, label_pred = model(img,ori_img_shape=ori_img_shape, res_img_shape=res_img_shape,pad_img_shape=pad_img_shape)
...
def main():
    #这里的epochs_tested表示要对哪几个epoch进行测试
    epochs_tested=[7,]
